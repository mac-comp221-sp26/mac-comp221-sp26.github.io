...
1. Show that $f(n) = \log n \in O(n)$. 
    - *Note*: When the inequality is obvious is a matter of contentious debate (depends on the audience!), but interpreting logarithms is strange but interpreting exponents is easier. If there is something that looks provable via induction like in Fleck, you can stop for now!  
