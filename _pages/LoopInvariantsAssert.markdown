...
def insert(arr : np.array, i : int):
    ...
    while (j > 0) and (arr[j] < arr[j-1]): 
        ...
        arr[j-1] = temp
        j -= 1
        print("\t After j = {}, \t arr[:i+1] = \t{}".format(j, arr[:i+1]))

def insertionSort(arr : np.array) -> None:
    for i in range(1, len(arr)):
        print("\ni = {}".format(i))
        insert(arr, i)
        print("\nAfter i = {},\t\t arr = \t\t{}".format(i, arr))
```
...
def insert(arr : np.array, i : int):
    ...
    while (j > 0) and (arr[j] < arr[j-1]): 
        ...
        arr[j-1] = temp
        j -= 1
        assert isSorted(arr, j, i+1), "Insert LI False" # Loop invariant for insert

def insertionSort(arr : np.array) -> None:
    for i in range(1, len(arr)):
        insert(arr, i)
        assert isSorted(arr, 0, i+1), "insertionSort LI False" # Our loop invariant for insertionSort
```
...
```python
assert isSorted(np.array([5,4,3,2,1]),0, 4), "This error message should appear"
```
    ...
    Cell In[45], line 1
    ----> 1 assert isSorted(np.array([5,4,3,2,1]),0, 4), "This error message should appear"
